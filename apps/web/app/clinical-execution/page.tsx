...
function Workspace() {
  ...
  return <main style={{ minHeight: "100vh", background: "#e9eef5", color: "#0f172a", padding: 12, fontFamily: "Inter, system-ui, sans-serif" }}>
    <header style={{ background: "#071019", color: "white", borderRadius: 17, padding: 17 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}><div><span style={{ color: "#2dd4bf", fontWeight: 900, fontSize: 11, letterSpacing: ".12em" }}>GOVERNED CLINICAL EXECUTION</span><h1 style={{ margin: "5px 0", fontSize: "clamp(34px,7vw,66px)", lineHeight: .95 }}>Patient work</h1></div><div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}><Link href="/patient-record" style={{ color: "white" }}>Patient record</Link><Link href="/patient-record/controlled-actions" style={{ color: "white" }}>Controlled actions</Link><Link href="/system-control" style={{ color: "white" }}>System control</Link></div></div>
...
function Medications({ data, busy, act }: any) {
  async function administration(row: any, status: string) {
    const reason = window.prompt("Reason:") || "Recorded by verified operator";
    const dose = status === "administered" ? window.prompt("Dose given:") : null;
    const witness = status === "administered" ? window.prompt("Witness subject/reference where required:") : null;
    await act(`/api/clinical-execution/administrations/${row.administrationRef}`, { expected_version: row.version, status, dose_given: dose, witness_subject: witness || null, omission_reason: status === "administered" ? null : reason, reason }, `Administration ${status}.`, "PATCH");
  }
  return <div style={{ display: "grid", gap: 9 }}>
    <section style={{ ...panel, borderColor: "#38bdf8", display: "grid", gap: 8 }}>
      ...
      <p style={{ margin: 0, color: "#475569" }}>Direct medication ordering is retired. Record the patient’s current weight and allergies, run the species/route/indication safety review in the longitudinal record, then issue the reviewed prescription through Controlled actions.</p>
    ...
    <section style={{ display: "grid", gap: 8 }}><h2>Administrations</h2>{data.administrations.length ? data.administrations.map((row: any) => <article key={row.administrationRef} style={{ ...panel, borderColor: row.status === "due" && new Date(row.scheduledAt) < new Date() ? "#ef4444" : "#cbd5e1" }}><strong>{row.orderRef}</strong><p>{new Date(row.scheduledAt).toLocaleString()} · {row.status}</p><div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}><button disabled={busy || row.status !== "due"} style={button} onClick={() => void administration(row, "administered")}>Administer</button><button disabled={busy || row.status !== "due"} style={{ ...button, background: "#a16207" }} onClick={() => void administration(row, "omitted")}>Omit</button><button disabled={busy || row.status !== "due"} style={{ ...button, background: "#475569" }} onClick={() => void administration(row, "withheld")}>Withhold</button></div></article>) : <article style={panel}>No scheduled administrations for the selected scope.</article>}</section>
  </div>;
}
