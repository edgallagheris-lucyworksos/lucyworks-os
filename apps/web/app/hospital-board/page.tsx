...
import { HospitalCommandCentre } from "@/components/hospital-command-centre";
...
type BoardView = "command" | "schedule" | "staffing";

export default function HospitalBoardPage() {
  const [view, setView] = useState<BoardView>("command");
        ...
        <div className="hospital-shell">
          ...
          <header className="hospital-shell__header">
            ...
            <nav className="hospital-shell__views" aria-label="Hospital board views">
              <button type="button" className={view === "command" ? "active" : ""} onClick={() => setView("command")}>Command centre</button>
              <button type="button" className={view === "schedule" ? "active" : ""} onClick={() => setView("schedule")}>Schedule control</button>
              <button type="button" className={view === "staffing" ? "active" : ""} onClick={() => setView("staffing")}>Staff detail</button>
            </nav>
            <nav className="hospital-shell__links" aria-label="Hospital navigation">
              ...
              <Link href="/workspace">Patients</Link>
          ...
          <main>
            {view === "command" ? <HospitalCommandCentre /> : null}
            {view === "schedule" ? <ResponsiveHospitalBoardV15 /> : null}
            {view === "staffing" ? <section className="hospital-shell__staff"><StaffLocationGrid /></section> : null}
          </main>
        </div>
      )}
    </AuthGuard>
...
@media(max-width:1120px){.hospital-shell__header{grid-template-columns:1fr auto;padding:9px 12px;gap:9px}.hospital-shell__links{grid-column:1/-1;justify-content:flex-end;border-top:1px solid #edf0f3;padding-top:6px}.hospital-shell__links .hospital-shell__user{margin-right:auto;margin-left:0;padding-left:0;border-left:0}}
@media(max-width:720px){.hospital-shell__identity h1{font-size:15px}.hospital-shell__identity>div:last-child>span{font-size:9px}.hospital-shell__mark{width:31px;height:31px}.hospital-shell__views{overflow:auto}.hospital-shell__views button{font-size:10px;padding:6px 8px;white-space:nowrap}.hospital-shell__links a:not(.primary){display:none}.hospital-shell__staff{padding:8px}.hospital-shell__user{display:none}.hospital-shell__staff .slg>.commandStrip{grid-template-columns:1fr 1fr!important}}
`;
