...
type GovernancePatch = Pick<ScheduledWorkBlock, "consentStatus" | "estimateStatus" | "insuranceStatus" | "pharmacyReady" | "ownerUpdated" | "referringVetReportSent" | "dischargeClear">;
...
function block(args: { id: string; time: string; lane: ScheduledWorkBlock["lane"]; what: string; who: string; where: string; how: string; blocker?: string; next: string; subject: string; caseId: string; duration: number; ownerRole?: string; ownerName?: string; resourceName?: string; governance?: GovernancePatch }): ScheduledWorkBlock {
  return { id: args.id, time: args.time, lane: args.lane, what: args.what, who: args.who, where: args.where, how: args.how, status: args.blocker ? "amber" : "green", blocker: args.blocker || "none", next: args.next, route: "/hospital-board", subject: args.subject, durationMinutes: args.duration, generatedFrom: "referral-pathway", episodeRef: args.caseId, assignedRole: args.ownerRole, assignedStaffName: args.ownerName, resourceName: args.resourceName, ...args.governance };
}
...
function procedureStep(item: ProcedureCatalogueItem, args: GenerateReferralPathwayArgs): ScheduledWorkBlock[] {
  ...
  return [
    block({ id: `${pathway}-triage`, time: t0, lane: "arrival", what: "Referral triage", who: "triage nurse", where: "front desk / triage", how: "confirm referral reason and urgency", next: "clinical review", subject: args.subject, caseId: pathway, duration: 15, ownerRole: "triage nurse", governance: { insuranceStatus: "pending" } }),
    block({ id: `${pathway}-consent-estimate`, time: addMinutes(t0, 15), lane: "insurance", what: "Consent and estimate gate", who: "admin", where: "admin queue", how: "confirm consent, estimate, insurance and owner understanding", blocker: "consent / estimate must be confirmed before procedure", next: item.label, subject: args.subject, caseId: pathway, duration: item.referralAdminMinutes || 20, ownerRole: "admin", governance: { consentStatus: "pending", estimateStatus: "pending", insuranceStatus: "pending" } }),
    block({ id: `${pathway}-procedure`, time: addMinutes(t0, 45), lane: item.area.includes("theatre") ? "rooms" : item.area.includes("imaging") ? "imaging" : "consult", what: item.label, who: item.accountableRole, where: item.resourceType, how: item.safetyChecks.join(" / "), next: "handover / recovery / owner update", subject: args.subject, caseId: pathway, duration: item.defaultMinutes, ownerRole: args.ownerRole || item.accountableRole, ownerName: args.ownerName, resourceName: item.resourceType, governance: { consentStatus: "pending", estimateStatus: "pending", insuranceStatus: "pending", pharmacyReady: item.pharmacyRefs.length ? false : true } }),
    block({ id: `${pathway}-pharmacy`, time: addMinutes(t0, 45), lane: "supply", what: "Pharmacy preparation", who: "pharmacy", where: "pharmacy", how: pharmacy || "no medication dependency", blocker: item.pharmacyRefs.length ? "pharmacy dependency must be prepared" : undefined, next: "confirm medication availability", subject: args.subject, caseId: pathway, duration: 15, ownerRole: "pharmacy", governance: { pharmacyReady: item.pharmacyRefs.length ? false : true } }),
