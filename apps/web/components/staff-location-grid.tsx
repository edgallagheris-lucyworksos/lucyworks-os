...
function procedure(block: ScheduledWorkBlock) {
  return procedureForWork(block.what, block.lane);
}
...
function cellBlocks(blocks: ScheduledWorkBlock[], time: string, column: ColumnKey, view: ViewKey) {
  if (view === "pharmacy" && column === "pharmacy") return blocks.filter((block) => block.time === time && (columnKey(block) === "pharmacy" || hasPharmacyDependency(block)));
  return blocks.filter((block) => block.time === time && columnKey(block) === column);
}
