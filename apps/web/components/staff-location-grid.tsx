...
type ViewKey = "all" | "blocked" | ColumnKey;
...
function viewColumns(view: ViewKey) {
  if (view === "all" || view === "blocked") return columns;
...
export function StaffLocationGrid() {
  ...
  return <main className="slg"><style>{css}</style><header><div><span>LucyWorks OS</span><h1>Staff location grid</h1><p>Time down the side. People and working groups across the top. Cells show patient, task, owner, location, blocker and next physical move.</p></div><aside><b>{blocked.length + integrityWarnings}</b><small>blocked / incomplete</small><button onClick={resetBlocks}>Reset</button></aside></header><nav>{views.map((item) => <button key={item.key} className={view === item.key ? "active" : ""} onClick={() => setView(item.key)}>{item.label}</button>)}</nav><section className="summary"><div><b>Pressure</b><small>{pressure.length} rows</small></div><div><b>Visible</b><small>{filteredBlocks.length} tasks</small></div><div><b>Integrity</b><small>{integrityWarnings} missing owner/location/next</small></div></section><ScheduleWarningsPanel /><section className="gridWrap"><section className="grid" style={{ gridTemplateColumns: `84px repeat(${visibleColumns.length}, minmax(220px, 1fr))`, minWidth: `${84 + visibleColumns.length * 220}px` }}><div className="corner">Time</div>{visibleColumns.map((column) => <div className="head" key={column.key}>{column.label}</div>)}{visibleTimes.map((time) => <div className="row" key={time}><div className="time">{time}</div>{visibleColumns.map((column) => { const cell = filteredBlocks.filter((block) => block.time === time && columnKey(block) === column.key); return <div className="cell" key={`${time}-${column.key}`}>{cell.length ? cell.map((block) => <button key={block.id} className={`work ${block.status}${integrityClass(block)}`} onClick={() => setSelected(target(block))}><b>{block.subject || block.what}</b><span>{block.what}</span><small>{owner(block) || "NO OWNER"} · {block.where || "NO LOCATION"}</small><em>{block.blocker !== "none" ? block.blocker : movement(block)}</em></button>) : <span className="empty">·</span>}</div>; })}</div>)}</section></section><section className="rule"><b>Rule:</b> staff, time and location first. Department pages are filtered views only. Missing owner, location or next action is treated as operationally unsafe.</section><QueueDetailDrawer target={selected} onClose={() => setSelected(null)} onActionComplete={onActionComplete} /></main>;
}
