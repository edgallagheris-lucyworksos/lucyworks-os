...
function MobileHospitalAgendaV15() {
  ...
  if (!board) return <main className="ha loading"><style>{css}</style>{status}</main>;
  ...
  return <main className="ha">
    <style>{css}</style>
    <header className="hero"><span>LucyWorks OS · mobile hospital agenda v15 + automation v23</span><h1>Hospital today</h1><p>Patients, places, times, staff, blockers and recorded automation evidence in one phone view.</p><nav><Link href="/workspace">Patient command</Link><Link href="/referral-intake">New referral</Link><Link href="/input">Quick input</Link><Link href="/automation-control">Automation authority</Link></nav></header>
    ...
    <section className="toolbar">
      ...
      <button onClick={() => setDate(localOperationalDate())}>Today</button><button onClick={() => void refresh()}>Refresh</button><strong>{status}</strong>
    </section>

    <section className="kpis"><article><b>{board.summary.blocks}</b><small>care blocks</small></article><article><b>{board.summary.episodes}</b><small>active patients</small></article><article className={board.summary.redConflicts ? "red" : "green"}><b>{board.summary.redConflicts}</b><small>red conflicts</small></article><article className={board.summary.unassignedBlocks ? "amber" : "green"}><b>{board.summary.unassignedBlocks}</b><small>no lead assigned</small></article></section>
    ...
    <section className="agenda"><div className="head"><div><span>OPERATING PLAN</span><h2>{blocks.length ? "Scheduled care" : "No care blocks scheduled"}</h2></div><small>{blocks.length} shown</small></div>
      {blocks.length ? blocks.map(block => {
        const issues = conflicts.get(block.blockRef) || [];
        return <article id={block.blockRef} className={tone(issues.length ? "red" : block.riskLevel)} key={block.blockRef}><div className="time"><b>{clock(block.startsAt)}</b><span>{clock(block.endsAt)}</span></div><div className="body"><header><div><small>{block.areaName}</small><h3>{block.patientName || "Operational work"}</h3></div><span className={`pill ${tone(block.status)}`}>{label(block.status)}</span></header><p>{block.procedureName}</p><dl><div><dt>Lead</dt><dd>{block.leadStaffName || "Not assigned"}</dd></div><div><dt>Episode</dt><dd>{block.episodeRef || "Not linked"}</dd></div></dl>{issues.length ? <div className="issues"><b>Cannot proceed cleanly</b>{issues.slice(0, 4).map(issue => <span key={issue}>{issue}</span>)}</div> : null}<footer>{block.episodeRef ? <Link href={`/episode-command?episode=${block.episodeRef}`}>Episode command</Link> : null}<Link href="/workspace">Patient command</Link><Link href="/automation-control">Automation evidence</Link></footer></div></article>;
      }) : <article className="empty"><b>The board is empty for this date.</b><p>Create or accept a synthetic referral, then confirm its proposed care block appears here.</p><Link href="/referral-intake">Start synthetic referral</Link></article>}
    </section>

    <section className="unscheduled"><div className="head"><div><span>CAPACITY RISK</span><h2>Active patients without a block</h2></div><small>{unscheduled.length}</small></div>{unscheduled.length ? unscheduled.map(episode => <article key={episode.episodeRef}><div><b>{episode.patientName}</b><span>{label(episode.phase)} · {label(episode.ownerRole)}</span><small>{episode.nextAction || "Review and schedule the next safe step"}</small></div><Link href={`/episode-command?episode=${episode.episodeRef}`}>Open</Link></article>) : <div className="clear"><b>Every active patient has at least one care block today.</b></div>}</section>
  </main>;
}
