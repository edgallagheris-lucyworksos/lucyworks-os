...
export const responsiveBoardRoles = [
  "admin",
  "clinician",
  "clinical_director",
  "hospital_director",
...
export function ResponsiveHospitalBoardV15() {
  ...
  const refresh = useCallback(async () => {
    try {
      const data = await apiGet<Board>(
        `/api/v11/master-board/day?premises_ref=${encodeURIComponent(premisesRef)}&operational_date=${operationalDate}`,
      );
      setBoard(data);
      setSelected((current) =>
  ...
  const visibleBlocks = useMemo(() => {
    if (!board) return [];
    if (!normalizedQuery) return board.blocks;
  ...
  const visibleBlockRefs = useMemo(
    () => new Set(visibleBlocks.map((block) => block.blockRef)),
    [visibleBlocks],
  );

  const visibleEpisodes = useMemo(() => {
    if (!board || !normalizedQuery) return board?.episodes || [];
    return board.episodes.filter((episode) =>
      [
        episode.patientName,
        episode.patientRef,
        episode.episodeRef,
  ...
  async function applyDelay() {
    ...
    try {
      await apiPost(`/api/hospital-ops/blocks/${selected.blockRef}/delay`, {
        minutes: delayMinutes,
        expectedVersions,
        reason: "Live operational delay from hospital operations board",
    ...
    try {
      const result = await apiPost<{ ok: boolean; episode?: Episode; guard?: Guard }>(
        `/api/v9/episodes/${selectedEpisode.episodeRef}/transition`,
        {
          ...
          reason: `Phase advanced from hospital operations board to ${targetPhase}`,
        },
      );
      if (!result.ok) {
  ...
  if (!board) return <section className="mbLoading">Loading {siteName}: {status}</section>;
  ...
  return (
    <section className="mb">
      ...
      <header className="hero">
        <div>
          <span>{siteName} · operational control</span>
          <h2>15-minute operating grid</h2>
          <p>Rooms, departments, active cases, accountable staff, blockers and governed actions in one live plan.</p>
        </div>
        <nav>
          <button type="button" className="emergencyAction" onClick={() => setEmergencyOpen(true)}>Insert emergency</button>
      ...
      <section className="toolbar">
        <label className="search">
          Find work
          <input
            aria-label="Find patient or work"
            placeholder="Find patient, procedure, staff or episode"
            value={query}
      ...
      <section className="liveStrip">
        ...
        {board.liveWindow.blocks.some((block) => visibleBlockRefs.has(block.blockRef)) ? board.liveWindow.blocks.filter((block) => visibleBlockRefs.has(block.blockRef)).map((block) => (
        ...
        >
          ...
          {areas.map((area) => (
            <div className="areaHead" key={area.areaRef}>
              <b>{area.name || area.areaRef}</b>
              <small>{area.department || area.areaType} · capacity {area.capacity} · turnover {area.turnoverMinutes}m</small>
            </div>
          ))}
          {timeRows.map((minute) => (
            <div
          ...
          {visibleBlocks.filter((block) => areas.some((area) => area.areaRef === block.areaRef)).map((block) => {
      ...
      <section className="lower">
        ...
        <article className="panel episodes">
          ...
          {visibleEpisodes.slice(0, 40).map((episode) => (
            <button
      ...
      {selected ? (
        <aside className="drawer">
          ...
          <div className="preview">
            {Object.entries(selected.gates || {}).length ? Object.entries(selected.gates).map(([gate, evidence]) => (
              <small key={gate}><b>{gate.replaceAll("_", " ")}</b> — {typeof evidence === "string" ? evidence : JSON.stringify(evidence)}</small>
            )) : <small>No block-level gate evidence recorded.</small>}
          </div>
          <div className="actions">
            <button onClick={() => void patchSelected({ commandType: "MoveOperationalBlock", startsAt: shiftIso(selected.startsAt, -15), endsAt: shiftIso(selected.endsAt, -15), action: "moved block earlier", reason: "hospital operations board move" })}>−15 min</button>
            <button onClick={() => void patchSelected({ commandType: "MoveOperationalBlock", startsAt: shiftIso(selected.startsAt, 15), endsAt: shiftIso(selected.endsAt, 15), action: "moved block later", reason: "hospital operations board move" })}>+15 min</button>
          </div>
          ...
          <button onClick={() => void patchSelected({ commandType: "AssignStaff", leadStaffRef, leadStaffName, action: "assigned verified staff reference", reason: "hospital operations board assignment" })}>Save assignment</button>
