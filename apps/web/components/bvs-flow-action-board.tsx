...
export function BvsFlowActionBoard() {
  ...
  return <div className="flow"><style>{css}</style><header><div><span>BVS flow routed action loop</span><h1>LucyFlow patient movement</h1><p>Click a stage to send work to the correct role queue: clinical director, ops manager, theatre lead, imaging, ICU/ward, owner comms, insurance or pharmacy.</p></div><nav><Link href="/hospital-board">Daily</Link><Link href="/resources">Resources</Link><Link href="/lucy-clinical">Clinical</Link></nav></header><main><section className="kpis"><div><span>Stages</span><strong>{bvsFlowStages.length}</strong></div><div><span>Rule</span><strong>Role queue + audit</strong></div></section><section className="grid">{bvsFlowStages.map((stage) => <button key={stage.id} type="button" onClick={() => setSelected(target(stage))} className="card"><b>{stage.label}</b><span>{stage.lane} · {sourceLabel(stage.source)}</span><p>{stage.commonBlockers.join(" · ")}</p><small>Next: {stage.nextActions[0]}</small></button>)}</section></main><QueueDetailDrawer target={selected} onClose={() => setSelected(null)} /></div>;
}
