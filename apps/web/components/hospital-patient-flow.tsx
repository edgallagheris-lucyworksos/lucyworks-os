...
type Episode = { episodeRef: string; patientRef?: string; patientName: string; phase: string; urgency: string; ownerRole: string; currentAreaRef?: string; nextAction?: string; status?: string };
...
export function HospitalPatientFlow({ onOpenResourceGrid }: { onOpenResourceGrid: () => void }) {
  ...
  const rows = useMemo<PatientRow[]>(() => {
    ...
    return board.episodes
      ...
      .map((episode) => {
        ...
        const risk = conflicts.some((conflict) => conflict.severity.toLowerCase() === "red")
          ? "red"
          : current ? normalRisk(current.riskLevel) : episode.urgency.toLowerCase() === "urgent" ? "amber" : "green";
        return { episode, areaName: areaNames.get(episode.currentAreaRef || "") || current?.areaName || "Location not recorded", current, next, conflicts, blocked, risk };
      });
  }, [board]);
  ...
  if (!board) return <section className="patientFlow loading"><style>{css}</style><b>{siteName}</b><span>{status}</span><button onClick={() => void refresh()}>Retry</button></section>;

  return <main className="patientFlow">
    <style>{css}</style>
    <header className="flowTitle">
      <div><span>Canonical patient flow</span><h2>Active patients</h2><p>One row per episode. Current location, timed work, accountable owner, conflict and next action.</p></div>
    ...
    <section className="flowTableWrap">
      <table>
        <thead><tr><th>Priority</th><th>Patient / episode</th><th>Current stage</th><th>Location</th><th>Accountable owner</th><th>Next timed work</th><th>Blocker / conflict</th><th></th></tr></thead>
