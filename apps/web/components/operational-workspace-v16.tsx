...
type AutomationWork = { id: number; title: string; ownerRole: string; urgency: string; status: string };
type AutomationTrigger = { triggerRef: string; episodeRef?: string | null; sourceType: string; sourceRef: string; mode: string; status: string; decisionOutcome?: string | null; workItems: AutomationWork[]; errorCode?: string | null; createdAt?: string | null };
type AutomationOverview = { configuration: { mode: string }; summary: { count: number; failed: number; active: number; workItems: number }; triggers: AutomationTrigger[] };
...
function tone(value: string) { const v = value.toLowerCase(); return ["red", "blocked", "overdue", "emergency", "failed", "governed_commit"].includes(v) ? "red" : ["amber", "urgent", "in_progress", "pending", "previewed", "preview_only", "queued", "processing"].includes(v) ? "amber" : "green"; }
...
export function OperationalWorkspaceV16() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      try {
        setAutomation(await apiGet<AutomationOverview>(`/api/v23/automation/overview?premises_ref=${PREMISES}&limit=500`));
      } catch {
        setAutomation(null);
      }
  ...
  const automationByEpisode = useMemo(() => {
    const map = new Map<string, AutomationTrigger[]>();
    for (const trigger of automation?.triggers || []) {
      if (!trigger.episodeRef) continue;
      map.set(trigger.episodeRef, [...(map.get(trigger.episodeRef) || []), trigger]);
    }
    return map;
  }, [automation]);
  ...
  return <main className="pc"><style>{css}</style>
    <header className="hero"><div><span>LUCYWORKS OS · PATIENT COMMAND V16 + AUTOMATION V23</span><h1>Patient Command</h1><p>Every patient answers who, what, where, when and how. Automation is shown as recorded review work, never as a clinical decision.</p></div><nav><Link href="/referral-intake">New referral</Link><Link href="/input">Quick input</Link><Link href="/hospital-board">Hospital today</Link><Link href="/automation-control">Automation authority</Link><Link href="/system-control">More tools</Link></nav></header>
    ...
    {view === "patients" ? <section className="list"><div className="head"><div><span>WHO · WHAT · WHERE · WHEN · HOW</span><h2>Patient flow</h2></div><small>{patients.length} active</small></div>{patients.length ? patients.map(patient => {
      ...
      const patientAutomation = automationByEpisode.get(patient.episodeRef) || [];
      const latestAutomation = patientAutomation[0];
      const generatedWork = patientAutomation.reduce((sum, item) => sum + item.workItems.length, 0);
      const failedAutomation = patientAutomation.filter(item => item.status === "failed").length;
      return <article className={`patient ${patient.attention.length || failedAutomation ? "attention" : ""}`} key={patient.episodeRef}>
        ...
        {latestAutomation ? <section className={`automation ${tone(latestAutomation.status)}`}><div><b>Automation evidence</b><span>{label(latestAutomation.sourceType)} · {label(latestAutomation.status)} · {label(latestAutomation.mode)}</span><small>{generatedWork} accountable work item{generatedWork === 1 ? "" : "s"}{failedAutomation ? ` · ${failedAutomation} failed trigger${failedAutomation === 1 ? "" : "s"}` : ""}</small></div><Link href="/automation-control">Review authority and history →</Link></section> : <section className="automation muted"><div><b>Automation evidence</b><span>No recorded trigger for this episode</span><small>Clinical responsibility remains with the named team.</small></div></section>}
