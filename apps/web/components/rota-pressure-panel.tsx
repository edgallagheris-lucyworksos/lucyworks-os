...
import { scheduledWorkBlocks } from "@/lib/day-control-work";

const pressure = scheduledWorkBlocks.filter((block) => block.lane === "breaks" || block.blocker !== "none");

export function RotaPressurePanel() {
  return <section className="rpp"><style>{css}</style><header><span>Day-grid pressure</span><h2>Staff cover and blocker pressure</h2><p>These rows come from the same 15-minute day grid, so rota pressure is not a separate story.</p></header><div>{pressure.map((block) => <Link key={block.id} href={block.route}><b>{block.time} · {block.what}</b><span>{block.lane} · {block.who}</span><small>{block.blocker} → {block.next}</small></Link>)}</div></section>;
}
