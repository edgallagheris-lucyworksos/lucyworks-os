...
import { readSpeechAuthority, SPEECH_NOTICE_VERSION } from "@/lib/speech-authority";
...
export function SpeechCaptureV19({
  ...
  useEffect(() => {
    ...
    const authority = readSpeechAuthority(session?.user, premisesRef);
    setNotice(Boolean(authority));
    setAuthorityName(authority ? `${authority.userName} · ${authority.role.replaceAll("_", " ")}` : "");
  }, [premisesRef]);
  ...
  function startRecording() {
    ...
    if (!notice) { setError("Confirm identity and recording authority before using the microphone."); return; }
    const candidate = window as typeof window & { SpeechRecognition?: RecognitionConstructor; webkitSpeechRecognition?: RecognitionConstructor };
  ...
  async function createDraft() {
    ...
    if (!notice) { setError("Confirm identity and recording authority before creating a speech draft."); return; }
  ...
  return <section className={`sp ${compact ? "compact" : ""}`}>
    <style>{css}</style>
    <header className="sp-head"><div><span>LUCYWORKSAI · GOVERNED SPEECH V19</span><h2>{createClinicalNote ? "Dictate, review, confirm" : "Speak, review, create owned work"}</h2><p>Speech becomes a proposed draft. The transcript, edits and final confirmation remain separate evidence.</p></div><b className={recording ? "recording" : notice ? "idle" : "blocked"}>{recording ? "● RECORDING" : notice ? "MIC READY" : "SETUP REQUIRED"}</b></header>

    <div className="identity"><b>Episode</b><span>{episodeRef || "No episode selected"}</span>{response?.context ? <><b>Patient</b><span>{response.context.patientName} · {label(response.context.phase)}</span></> : null}<b>Speech authority</b><span>{authorityName || "Not confirmed"}</span></div>
    ...
    {!response ? <>
      <div className="settings">
        ...
        {notice ? <div className="notice authority-ok"><span><b>Identity & recording authority confirmed</b><br />{authorityName} · {siteName}<br /><small>Raw audio retention is off.</small></span><Link href="/speech-authority?returnTo=%2Finput">Review authority →</Link></div> : <div className="notice authority-needed"><span><b>Before using speech capture, confirm who you are and your recording authority.</b><br /><small>The microphone stays locked until this is resolved.</small></span><Link href="/speech-authority?returnTo=%2Finput">Confirm identity & recording authority →</Link></div>}
      </div>
      <div className="mic-row"><button className={recording ? "stop" : "primary"} onClick={recording ? stopRecording : startRecording} disabled={!episodeRef.trim() || !notice}>{recording ? "Stop recording" : recognitionAvailable ? "Start microphone" : "Microphone unavailable"}</button><button onClick={() => { setTranscript(""); setPartial(""); }}>Clear</button></div>
