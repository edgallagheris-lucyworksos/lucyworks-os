...
function roomName(room: { room_name?: string; name?: string } | undefined) { return room?.room_name || room?.name || ""; }
function laneHref(lane: string) { return lane === "Insurance" || lane === "Owner comms" ? "/lucy-comms" : lane === "Pharmacy" ? "/lucy-pharm" : lane === "Lab" ? "/lucy-clinical" : lane === "Theatre" || lane === "Recovery" || lane === "ICU" ? "/resources" : "/flow"; }
...
function PressureStrip({ dashboard, pulse }: { dashboard: DashboardData | null; pulse: PulseData | null }) {
  ...
  const cards = [
    ...
    ["Theatre pressure", "11 rooms", red ? "amber" : "green"],
    ["Diagnostics", "MRI/CT/X-ray", n(pulse?.pending_result_reviews, 0) ? "amber" : "blue"],
...
function TheatreComplex({ dashboard }: { dashboard: DashboardData | null }) {
  return <div className="theatre-grid">{theatreUnits.map((unit, index) => {
    ...
    return <Link href="/resources" className={`theatre-card ${statusClass(status)}`} key={unit.id}><b>{unit.label}</b><strong>{state}</strong><span>{caseRef}</span><small>{block?.next_action?.detail || unit.blockers[0]}</small></Link>;
  })}</div>;
}
...
function UnitCard({ unit, pulse }: { unit: OperatingUnit; pulse: PulseData | null }) {
  ...
  return <Link href={unit.route} className={`unit-card ${statusClass(status)}`}><b>{unit.label}</b><span>{unit.ownerRole}</span><small>{unit.tracks.slice(0, 4).join(" · ")}</small><em>{worst?.next_action || worst?.detail || unit.blockers[0]}</em></Link>;
}
...
export function DailyOperationalBoard() {
  ...
  return <div className="daily-board"><style>{css}</style><header className="daily-hero"><div><span>BVS-scale daily operations · {updated}</span><h1>Daily operational control</h1><p>Live daily board using dashboard intelligence and conflict pulse when available, with safe fallback data when the API is offline.</p></div><div className="daily-hero-actions"><Link href="/interrupts">Open Pulse</Link><Link href="/resources">Resource control</Link><Link href="/lucy-gov">Audit trail</Link></div></header><PressureStrip dashboard={dashboard} pulse={pulse} /><div className="daily-layout"><main><Section label="theatre complex" title="11-theatre operating grid"><TheatreComplex dashboard={dashboard} /></Section><Section label="hospital units" title="Diagnostics, pharmacy, insurance, beds and support"><OperatingUnits pulse={pulse} /></Section><Section label="timeline" title="Now to +120 minutes"><Timeline dashboard={dashboard} /></Section></main><aside><Section label="priority" title="Decision and action rail"><ActionRail pulse={pulse} /></Section><Section label="audit" title="Latest operating decisions"><div className="audit-list"><Link href="/lucy-gov">{n(dashboard?.summary?.conflicts, 0)} conflicts tracked</Link><Link href="/lucy-gov">{n(dashboard?.summary?.alerts, 0)} alerts active</Link><Link href="/resources">{n(dashboard?.summary?.schedule_blocks, 0)} schedule blocks today</Link><Link href="/flow">{n(dashboard?.summary?.active_slots, 0)} active slots</Link></div></Section></aside></div></div>;
}
