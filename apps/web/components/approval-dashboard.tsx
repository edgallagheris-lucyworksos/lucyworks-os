...
export function ApprovalDashboard() {
  ...
  const [note, setNote] = useState("reviewed; responsibility retained by named approver");

  async function refresh() {
    try {
      const query = filter === "all" ? "" : `?status=${filter}`;
      const response = await fetch(`${API_BASE}/api/evidence/approvals${query}`, { cache: "no-store" });
  ...
  async function decide(item: ApprovalTask, decision: "approved" | "rejected") {
    setStatus(`${decision}...`);
    try {
      await decideApproval(item.id, { decision, decidedBy, decidedByRole, note });
      await refresh();
  ...
  return <main className="approvalDash"><style>{css}</style>
    ...
    <section className="controls"><label>Filter<select value={filter} onChange={(event) => setFilter(event.target.value)}><option value="pending">pending</option><option value="approved">approved</option><option value="rejected">rejected</option><option value="all">all</option></select></label><label>Approver<input value={decidedBy} onChange={(event) => setDecidedBy(event.target.value)} /></label><label>Role<select value={decidedByRole} onChange={(event) => setDecidedByRole(event.target.value)}><option value="clinical_director">clinical_director</option><option value="ops_manager">ops_manager</option><option value="senior_clinician">senior_clinician</option><option value="hospital_director">hospital_director</option><option value="supervisor">supervisor</option><option value="governance_lead">governance_lead</option><option value="clinician">clinician - blocked by API</option></select></label><label>Decision note<textarea value={note} onChange={(event) => setNote(event.target.value)} /></label><button onClick={() => void refresh()}>Refresh</button><small>{status}</small></section>
