...
TEST_DB = Path(tempfile.gettempdir()) / f"lucyworks_backend_smoke_{os.getpid()}.db"
if TEST_DB.exists():
    TEST_DB.unlink()
...
try:
    with TestClient(app) as client:
        ...
        triage_payload = triage_list[0]
        urgency = triage_payload.get("urgency") or triage_payload.get("triage_result")
        assert urgency in {"red", "amber", "green", "emergency"}, triage_payload
        triage_id = triage_payload["id"]
