...
from app.safety_control_v25_schemas import (
    ClosePayload,
    ClosureReviewPayload,
    ConflictPayload,
    EscalationCreate,
    OwnerAssignmentPayload,
    OwnershipDecisionPayload,
    ReopenPayload,
    SafetyActionComplete,
    SafetyActionCreate,
    SafetyActionVerify,
    SafetyRecordCreate,
    TriagePayload,
...
def _bundle(session: Session, record: SafetyRecordV25) -> dict[str, Any]:
    actions = session.exec(
        select(SafetyActionV25)
        .where(SafetyActionV25.record_ref == record.record_ref)
        .order_by(SafetyActionV25.created_at)
    ).all()
    decisions = session.exec(
        select(SafetyDecisionV25)
        .where(SafetyDecisionV25.record_ref == record.record_ref)
        .order_by(SafetyDecisionV25.created_at.desc())
    ).all()
    escalations = session.exec(
        select(SafetyEscalationV25)
        .where(SafetyEscalationV25.record_ref == record.record_ref)
        .order_by(SafetyEscalationV25.created_at.desc())
    ).all()
    links = session.exec(
        select(SafetyLinkV25)
        .where(SafetyLinkV25.record_ref == record.record_ref)
        .order_by(SafetyLinkV25.created_at)
    ).all()
    return {
...
def board_indicators(
    ...
    query = (
        select(SafetyRecordV25)
        .where(SafetyRecordV25.status.in_(OPEN_RECORD_STATUSES))
        .order_by(SafetyRecordV25.updated_at.desc())
    )
    if premises_ref:
...
def triage_record(
    ...
    if payload.confidentiality is not None:
        confidentiality = payload.confidentiality.lower().strip()
        if confidentiality not in CONFIDENTIALITY:
            raise HTTPException(status_code=400, detail="invalid confidentiality")
        if record.record_type in {"conduct", "bullying", "harassment", "retaliation", "grievance"} and confidentiality != "strict":
    ...
    if payload.status is not None:
        ...
        if status not in OPEN_RECORD_STATUSES:
    ...
    decision = create_decision(
        ...
        result_state=sensitive_record_dict(record),
    )
    session.commit()
...
def ownership_decision(
    ...
    decision = create_decision(
        ...
        record,
        auth,
        decision_type="ownership_response",
    ...
    if decision_value == "rejected":
        escalation = create_escalation(
            ...
            auth,
            {
                "reason": f"Named owner rejected responsibility: {payload.reason}",
...
def declare_conflict(
    ...
    escalation = create_escalation(
        ...
        record,
        auth,
        {"reason": f"Conflict declared: {payload.reason}", "to": {"role": "governance_lead"}},
    )
    session.commit()
    return {
        "decision": decision_dict(decision),
        "escalation": escalation_dict(escalation),
        **_bundle(session, record),
    }
    ...
    return {
        "counts": {
            ...
            "unowned": sum(
                ...
                if not (
                    row.accountable_owner_subject
                    or row.clinical_owner_subject
                    or row.independent_owner_subject
                )
            ),
        },
        "records": [
            public_record_dict(row) if row.confidentiality != "standard" else sensitive_record_dict(row)
...
def get_access_log(
    ...
    rows = session.exec(
        select(SafetyAccessEventV25)
        .where(SafetyAccessEventV25.record_ref == record_ref)
        .order_by(SafetyAccessEventV25.created_at.desc())
    ).all()
    return {
