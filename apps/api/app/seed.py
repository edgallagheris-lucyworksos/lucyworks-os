...
def seed_data(session: Session) -> None:
    ...
    procedure_by_name = {proc.name: proc for proc in session.exec(select(ProcedureType)).all()}
    schedule_start = now + timedelta(minutes=30)
    cp_tplo = CaseProcedure(episode_id=episode_by_ref["EP-1056"].id, procedure_type_id=procedure_by_name["TPLO"].id, status="planned", scheduled_start=schedule_start)
    session.add(cp_tplo)
    session.commit()
    session.refresh(cp_tplo)

    slot = schedule_start
    for block_type, mins in [("prep", 20), ("anaesthesia", 15), ("procedure", 90), ("recovery", 45), ("cleaning", 20)]:
        block = ScheduleBlock(episode_id=episode_by_ref["EP-1056"].id, case_procedure_id=cp_tplo.id, block_type=block_type, room_name="Theatre 1", owner_role="clinician", starts_at=slot, ends_at=slot + timedelta(minutes=mins), status="planned")
        session.add(block)
        slot = slot + timedelta(minutes=mins)
    session.commit()
