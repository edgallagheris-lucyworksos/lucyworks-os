...
def create_triage(payload: dict, session: Session = Depends(get_session)):
    ...
    if triage.ethics_triggered:
        ...
        session.add(flag)
    session.commit(); log(session,"LucyFlow","created","triage_assessment",triage.id or 0,triage.reasoning); return {"triage":row_dict(triage),"work_item":row_dict(work),"decision":row_dict(decision)}
...
def resolve_triage(triage_id:int, note:str="Resolved", session: Session = Depends(get_session)):
    ...
    triage.status="resolved"; triage.resolved_at=datetime.now(timezone.utc); session.add(triage); session.commit(); session.refresh(triage); log(session,"LucyFlow","resolved","triage_assessment",triage_id,note); return row_dict(triage)
...
def create_ethics(payload: dict, session: Session = Depends(get_session)):
    ...
    flag.linked_work_item_id=work.id; session.add(flag); session.commit(); session.refresh(flag); session.refresh(work); log(session,"Lucy Ethics","created","ethics_flag",flag.id or 0,flag.detail); return {"ethics_flag":row_dict(flag),"work_item":row_dict(work)}
...
def resolve_ethics(flag_id:int, note:str="Resolved", session: Session = Depends(get_session)):
    ...
    flag.status="resolved"; flag.resolved_at=datetime.now(timezone.utc); flag.resolution_note=note; session.add(flag); session.commit(); session.refresh(flag); log(session,"Lucy Ethics","resolved","ethics_flag",flag_id,note); return row_dict(flag)
...
def create_care(payload: dict, session: Session = Depends(get_session)):
    task=LucyCareTask(**payload); session.add(task); session.commit(); session.refresh(task); log(session,"Lucy Care","created","care_task",task.id or 0,task.detail); return row_dict(task)
...
def complete_care(task_id:int, session: Session = Depends(get_session)):
    ...
    task.status="done"; task.completed_at=datetime.now(timezone.utc); session.add(task); session.commit(); session.refresh(task); log(session,"Lucy Care","completed","care_task",task_id,task.detail); return row_dict(task)
...
def create_decision(payload: dict, session: Session = Depends(get_session)):
    rec=DecisionRecord(**payload); session.add(rec); session.commit(); session.refresh(rec); log(session,"LucyWorks","created","decision",rec.id or 0,rec.decision_needed); return row_dict(rec)
...
def resolve_decision(decision_id:int, resolution:str="Resolved", session: Session = Depends(get_session)):
    ...
    rec.status="resolved"; rec.resolved_at=datetime.now(timezone.utc); rec.resolution=resolution; session.add(rec); session.commit(); session.refresh(rec); log(session,"LucyWorks","resolved","decision",decision_id,resolution); return row_dict(rec)
...
def create_blocker(payload: dict, session: Session = Depends(get_session)):
    blocker=Blocker(**payload); session.add(blocker); session.commit(); session.refresh(blocker); log(session,"LucyWorks","created","blocker",blocker.id or 0,blocker.detail); return row_dict(blocker)
...
def resolve_blocker(blocker_id:int, session: Session = Depends(get_session)):
    ...
    blocker.status="resolved"; blocker.resolved_at=datetime.now(timezone.utc); session.add(blocker); session.commit(); session.refresh(blocker); log(session,"LucyWorks","resolved","blocker",blocker_id,blocker.detail); return row_dict(blocker)
...
def create_escalation(payload: dict, session: Session = Depends(get_session)):
    esc=EscalationEvent(**payload); session.add(esc); session.commit(); session.refresh(esc); log(session,"LucyWorks","created","escalation",esc.id or 0,esc.reason); return row_dict(esc)
...
def resolve_escalation(escalation_id:int, session: Session = Depends(get_session)):
    ...
    esc.status="resolved"; esc.resolved_at=datetime.now(timezone.utc); session.add(esc); session.commit(); session.refresh(esc); log(session,"LucyWorks","resolved","escalation",escalation_id,esc.reason); return row_dict(esc)
...
def create_owner_comms(payload: dict, session: Session = Depends(get_session)):
    req=OwnerCommsRequirement(**payload); session.add(req); session.commit(); session.refresh(req); log(session,"Mail Ops","created","owner_comms_requirement",req.id or 0,req.reason); return row_dict(req)
...
def complete_owner_comms(requirement_id:int, session: Session = Depends(get_session)):
    ...
    req.status="complete"; req.completed_at=datetime.now(timezone.utc); session.add(req); session.commit(); session.refresh(req); log(session,"Mail Ops","completed","owner_comms_requirement",requirement_id,req.reason); return row_dict(req)
...
def create_pulse_signal(payload: dict, session: Session = Depends(get_session)):
    sig=PulseSignal(**payload); session.add(sig); session.commit(); session.refresh(sig); log(session,"Lucy Pulse","created","pulse_signal",sig.id or 0,sig.detail); return row_dict(sig)
...
def create_work(payload: WorkItemCreate, session: Session = Depends(get_session)):
    item=WorkItem(**payload.model_dump()); session.add(item); session.commit(); session.refresh(item); log(session,"System","created","work_item",item.id or 0,item.title); return row_dict(item)
...
def assign_work(item_id:int,payload:WorkItemAssign,session:Session=Depends(get_session)):
    ...
    item.owner_role=payload.owner_role; item.owner_user_id=payload.owner_user_id; session.add(item); session.commit(); session.refresh(item); log(session,payload.actor_name,"assigned","work_item",item.id or 0,f"Assigned to {payload.owner_role}"); return row_dict(item)
...
def work_status(item_id:int,payload:WorkItemStatusUpdate,session:Session=Depends(get_session)):
    item=session.get(WorkItem,item_id)
    if not item: raise HTTPException(status_code=404, detail="Work item not found")
    item.status=payload.status; item.updated_at=datetime.now(timezone.utc); session.add(item); session.commit(); session.refresh(item); log(session,payload.actor_name,"status_updated","work_item",item.id or 0,payload.status); return row_dict(item)
...
def generate_schedule(payload:ScheduleGenerateRequest,session:Session=Depends(get_session)):
    ...
    ep.current_phase="scheduled"; ep.current_room_name=payload.room_name; ep.current_section_name=proc.department; session.add(ep); session.commit(); session.refresh(cp); [session.refresh(b) for b in blocks]; log(session,payload.actor_name,"schedule_generated","case_procedure",cp.id or 0,payload.episode_ref); return {"case_procedure":row_dict(cp),"blocks":[row_dict(b) for b in blocks]}
...
def get_staff_load(session:Session=Depends(get_session)):
    staff_members = session.exec(select(StaffMember)).all()
    blocks = session.exec(select(ScheduleBlock)).all()
    out = []
    for staff in staff_members:
        assigned = [b for b in blocks if b.assigned_staff_member_id == staff.id and b.status != "cancelled"]
        minutes = sum(int((b.ends_at - b.starts_at).total_seconds() // 60) for b in assigned)
        out.append({"staff_member_id": staff.id, "name": staff.name, "role": staff.role, "assigned_blocks": len(assigned), "assigned_minutes": minutes})
    return out
...
def conflict_to_work(conflict_type:str,severity:str,detail:str,session:Session=Depends(get_session)):
    ...
    action=ConflictAction(conflict_type=conflict_type,severity=severity,detail=detail,linked_work_item_id=item.id); session.add(action); session.commit(); session.refresh(action); session.refresh(item); log(session,"System","conflict_to_work","work_item",item.id or 0,conflict_type); return {"work_item":row_dict(item),"conflict_action":row_dict(action)}
...
def resolve_conflict(action_id:int,note:str="Resolved",session:Session=Depends(get_session)):
    ...
    action.status="resolved"; action.resolved_at=datetime.now(timezone.utc); action.resolution_note=note; session.add(action); session.commit(); session.refresh(action); log(session,"System","conflict_resolved","conflict_action",action.id or 0,note); return row_dict(action)
