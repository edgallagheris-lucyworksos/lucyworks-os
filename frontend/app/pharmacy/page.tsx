...
export default function PharmacyPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setItems(all.filter((x: WorkItem) => x.category === "pharmacy" || x.input_type === "pharmacy" || x.title.toLowerCase().includes("med") || x.title.toLowerCase().includes("drug")));
    }
    load();
  }, []);
  ...
  return (
    <AuthGuard allowedRoles={["ops_manager", "clinician", "nurse", "admin"]}>
      {() => (
        <HospitalShell title="Pharmacy" subtitle="Medication work, ordering flags and controlled handoff">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, marginBottom: 18 }}>
            <section style={{ border: "1px solid #78350f", borderRadius: 18, padding: 16, background: "#0f172a" }}><div style={{ color: "#94a3b8" }}>Urgent meds work</div><div style={{ fontSize: 34 }}>{urgent.length}</div></section>
            <section style={{ border: "1px solid #1f2937", borderRadius: 18, padding: 16, background: "#0f172a" }}><div style={{ color: "#94a3b8" }}>Open pharmacy work</div><div style={{ fontSize: 34 }}>{open.length}</div></section>
          ...
          <section style={{ border: "1px solid #1f2937", borderRadius: 18, overflow: "hidden" }}>
            <div style={{ padding: 16, background: "#0f172a", fontWeight: 700 }}>Pharmacy queue</div>
            {items.map((item) => (
            ...
            {!items.length ? <div style={{ padding: 16, color: "#94a3b8" }}>No pharmacy items returned.</div> : null}
          </section>
        </HospitalShell>
      )}
    </AuthGuard>
