...
function SpineSection({ title, items, empty, render }: { title: string; items: any[]; empty: string; render: (item: any, index: number) => React.ReactNode }) { return <section style={{ border: "1px solid #1f2937", borderRadius: 18, overflow: "hidden" }}><div style={{ padding: 16, background: "#0f172a", fontWeight: 700 }}>{title}</div>{items.map(render)}{!items.length ? <div style={{ padding: 16, color: "#94a3b8" }}>{empty}</div> : null}</section>; }
