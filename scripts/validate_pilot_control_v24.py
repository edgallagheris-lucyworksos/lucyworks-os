...
for path in (MODELS, SERVICE, ROUTES, READINESS_PATCH, MAIN, MIGRATION, SMOKE, RUNNER, PAGE, UI, SYSTEM_CONTROL, RESTORE):
...
for token in (
    ...
    "stop_criteria",
    "automation_mode",
    "accountable_owner_subject",
    "clinical_owner_subject",
):
...
for token in (
    ...
    "APPROVE PILOT CONTROL BOUNDARY",
    "INITIATE PILOT ROLLBACK",
    "record_approval",
    "independent_approval_missing",
    "clinical_owner_missing",
    "uat_incomplete",
    "open_red_observations",
    "red_shadow_mismatches",
...
for token in (
    'prefix="/api/v24/pilots"',
    "canonical_pilot_route_required",
    "legacy_shadow_guard_router",
...
), "v24 legacy shadow guard must be registered before caller-attributed legacy writes"
...
assert not (constructed & {
    ...
    "EpisodeTransitionV9",
    "EpisodeClosureV9",
}), f"v24 constructed forbidden clinical entities: {constructed}"
...
for table in (
    "PilotAuthorityV24.__table__.create",
    "PilotApprovalV24.__table__.create",
    "PilotControlActionV24.__table__.create",
    "PilotShadowComparisonV24.__table__.create",
    "PilotUATScenarioV24.__table__.create",
):
    assert table in migration_text, f"v24 migration missing: {table}"

for token in (
    "0018_bounded_pilot_control_v24",
    "pilotauthorityv24",
    "pilotapprovalv24",
    "pilotcontrolactionv24",
    "pilotshadowcomparisonv24",
    ...
    assert token in restore_text, f"v24 restore proof missing: {token}"

for proof in (
    "canonical_pilot_route_required",
    "stale_pilot_authority",
    "planVersion",
    "patient_identity_mismatch",
    '"severity": "red"',
    "pilotStopped",
    "AUTHORISE SHADOW MODE ONLY",
    "AUTHORISE BOUNDED LIVE PILOT WITH HUMAN CLINICAL AUTHORITY",
    "INITIATE PILOT ROLLBACK",
    'integrity["ok"] is True',
    "canonical.phase == \"referral_received\"",
    "MedicationOrder",
    "EpisodeTransitionV9",
):
...
for token in (
    "Hospital pilot authority",
    "STOP PILOT",
    "Rollback",
    "Current blockers",
    "Critical hospital journeys",
    "SHADOW COMPARISON",
